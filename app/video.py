...
async def fetch_sennin_video_info(v: str):
    ...
    try:
        ...
        raise Exception("Sennin video info timeout")
    except Exception:
        pass
...
async def fetch_sia_video(v: str):
    ...
    try:
        ...
        raise Exception("Sia video info timeout")
    except Exception:
        pass
...
async def fetch_video_info_invidious_robust(v: str, force_instance: str = None) -> dict:
    """Invidious APIから堅牢に動画情報取得（複数インスタンス対応）"""
    # 最初にデフォルトインスタンスを試行
    ...
    # フォールバック: 複数のInvidious公開インスタンスを試行
